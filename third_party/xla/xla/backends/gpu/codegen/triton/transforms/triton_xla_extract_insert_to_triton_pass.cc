...
template <typename T>
auto RemoveReducedDims(ArrayRef<T> values,
                       const llvm::SmallDenseSet<unsigned>& reduced_dims) {
  SmallVector<T> result;
  result.reserve(values.size() - reduced_dims.size());
...
SmallVector<int64_t> GetRankeReducedLayout(
    ArrayRef<int64_t> layout,
    const llvm::SmallDenseSet<unsigned>& reduced_dims) {
  auto vec = to_vector(reduced_dims);
  SmallVector<int64_t> result;
  result.reserve(layout.size() - vec.size());
  for (auto dim : layout) {
    auto it = absl::c_lower_bound(vec, dim);
    if (it != vec.end() && *it == dim) {
      continue;
    }
    result.push_back(dim - std::distance(vec.begin(), it));
  }
  return result;
}

std::pair<mlir::Value, Value> CreateTensorOfPointersAndMask(
    ...
    llvm::SmallDenseSet<unsigned> reduced_dims, ArrayRef<int64_t> tile_shape) {
  ...
  // Compute strides from the original shape.
  SmallVector<int64_t> shape_strides(original_shape.size());
  int64_t stride = 1;
  for (int64_t dim : layout) {
    shape_strides[dim] = stride;
    stride *= original_shape[dim];
  }

  // Compute the set of not-reduced dimensions.
  SmallVector<unsigned> retained_dims;
  retained_dims.reserve(original_shape.size() - reduced_dims.size());
  for (int dim = 0; dim < original_shape.size(); ++dim) {
    if (!reduced_dims.contains(dim)) {
      CHECK(sizes[dim] == tile_shape[retained_dims.size()]);

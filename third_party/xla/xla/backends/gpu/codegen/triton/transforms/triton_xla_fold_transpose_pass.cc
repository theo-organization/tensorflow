...
LogicalResult FoldTransposeOfExtract(TransOp op, PatternRewriter& rewriter) {
  ...
  std::optional<llvm::SmallDenseSet<unsigned>> mask = computeRankReductionMask(
      extract.getStaticSizes(), extract.getType().getShape());
  if (!mask) {
  ...
  retained_dims.reserve(extract.getSrcShape().size() - mask->size());
  for (int dim = 0; dim < extract.getSrcShape().size(); ++dim) {
  ...
  permutation.reserve(extract.getSrcShape().size());
  ...
  while (permutation.size() < extract.getSrcShape().size()) {
    permutation.push_back(permutation.size());
  }
